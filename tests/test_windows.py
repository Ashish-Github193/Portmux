...
from portmux.windows import create_window, kill_window, list_windows, window_exists
...
class TestCreateWindow:
    def test_create_window_success(self, mocker):
        ...
        mock_run.assert_called_once_with(
            ["tmux", "new-window", "-t", "portmux", "-n", "test-window", 
    ...
    def test_create_window_custom_session(self, mocker):
        ...
        assert result is True
        mock_run.assert_called_once_with(
            ["tmux", "new-window", "-t", "custom-session", "-n", "test-window", "echo test"],
            capture_output=True,
            text=True,
            check=False
        )
    ...
    def test_create_window_error(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="session not found: portmux"
        )
        
        with pytest.raises(TmuxError, match="Failed to create window 'test-window': session not found: portmux"):
    ...
    def test_create_window_tmux_not_found(self, mocker):
        ...
        with pytest.raises(TmuxError, match="tmux is not installed or not found in PATH"):
            create_window("test-window", "echo test")
...
class TestKillWindow:
    ...
    def test_kill_window_not_found(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="window not found: test-window"
        )
    ...
    def test_kill_window_cant_find(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="can't find window: test-window"
        )
    ...
    def test_kill_window_other_error(self, mocker):
        ...
        with pytest.raises(TmuxError, match="Failed to kill window 'test-window': some other error"):
...
class TestListWindows:
    ...
    def test_list_windows_custom_session(self, mocker):
        ...
        assert result == expected
        mock_run.assert_called_once_with(
            ["tmux", "list-windows", "-t", "custom-session", "-F", 
             "#{window_name}|#{window_flags}|#{pane_current_command}"],
            capture_output=True,
            text=True,
            check=False

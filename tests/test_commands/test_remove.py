...
class TestRemoveCommand:
    ...
    def test_remove_single_forward_success(self, mock_remove_forward, mock_list_forwards, mock_session_exists):
        ...
        result = self.runner.invoke(remove, ['L:8080:localhost:80'], 
    ...
    def test_remove_all_with_confirmation(self, mock_confirm, mock_remove_forward, 
                                         mock_list_forwards, mock_session_exists):
        mock_session_exists.return_value = True
        mock_list_forwards.return_value = [
            {'name': 'L:8080:localhost:80', 'direction': 'L', 'spec': '8080:localhost:80'},
        ...
        result = self.runner.invoke(remove, ['--all'], 
                                  obj={'session': 'portmux', 'config': None, 'verbose': False})
    ...
    @patch('portmux.commands.remove.confirm_destructive_action')
    def test_destroy_session_with_confirmation(self, mock_confirm, mock_kill_session, mock_session_exists):
        mock_session_exists.return_value = True
        mock_confirm.return_value = True
        mock_kill_session.return_value = True
        
        result = self.runner.invoke(remove, ['--destroy-session'], 
                                  obj={'session': 'portmux', 'config': None, 'verbose': False})

...
class TestInitCommand:
    ...
    def test_init_new_session_success(
        ...
        result = self.runner.invoke(
            init, [], obj={"session": "portmux", "config": None, "verbose": False, "output": Output()}
        )
    ...
    def test_init_creates_default_config(
        ...
        with patch("portmux.service.create_session") as mock_create_session:
            mock_create_session.return_value = True

            result = self.runner.invoke(
                init, [], obj={"session": "portmux", "config": None, "verbose": True, "output": Output()}
            )

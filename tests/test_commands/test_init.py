...
from portmux.exceptions import TmuxError, ConfigError
...
class TestInitCommand:
    ...
    def test_init_new_session_success(self, mock_create_config, mock_load_config, 
        ...
        result = self.runner.invoke(init, [], obj={'session': 'portmux', 'config': None, 'verbose': False})
    ...
    @patch('portmux.commands.init.session_exists')
    @patch('portmux.commands.init.load_config')
    def test_init_existing_session_no_force(self, mock_load_config, mock_session_exists):
    ...
    @patch('portmux.commands.init.load_config')
    def test_init_force_recreate(self, mock_load_config, mock_kill_session, 
                                mock_create_session, mock_session_exists):
        mock_session_exists.return_value = True
    ...
    @patch('portmux.commands.init.load_config')
    @patch('portmux.commands.init.create_default_config')
    def test_init_creates_default_config(self, mock_create_config, mock_load_config, mock_session_exists):
        mock_session_exists.return_value = False
        mock_load_config.side_effect = [Exception("Config not found"), {'session_name': 'portmux'}]
        
        with patch('portmux.commands.init.create_session') as mock_create_session:
            mock_create_session.return_value = True
            
            result = self.runner.invoke(init, [], obj={'session': 'portmux', 'config': None, 'verbose': True})
    ...
    @patch('portmux.commands.init.create_session')
    @patch('portmux.commands.init.load_config')
    def test_init_verbose_output(self, mock_load_config, mock_create_session, mock_session_exists):
        mock_session_exists.return_value = False
        mock_create_session.return_value = True
        mock_load_config.return_value = {'session_name': 'portmux'}

...
class TestRefreshCommand:
    ...
    def test_refresh_single_forward_success(self, mock_load_config, mock_refresh_forward, 
        ...
        result = self.runner.invoke(refresh, ['L:8080:localhost:80'], 
    ...
    @patch('portmux.commands.refresh.load_config')
    @patch('time.sleep')
    def test_refresh_all_forwards(self, mock_sleep, mock_load_config, mock_refresh_forward, 
                                 mock_list_forwards, mock_session_exists):
        mock_session_exists.return_value = True
        mock_list_forwards.return_value = [
            {'name': 'L:8080:localhost:80', 'direction': 'L', 'spec': '8080:localhost:80'},
        ...
        result = self.runner.invoke(refresh, ['--all'], 
                                  obj={'session': 'portmux', 'config': None, 'verbose': False})
    ...
    @patch('portmux.commands.refresh.refresh_forward')
    @patch('portmux.commands.refresh.load_config')
    def test_refresh_with_custom_delay(self, mock_load_config, mock_refresh_forward, 
                                      mock_list_forwards, mock_session_exists):
        mock_session_exists.return_value = True
        mock_list_forwards.return_value = [
            {'name': 'L:8080:localhost:80', 'direction': 'L', 'spec': '8080:localhost:80'}
        ...
        result = self.runner.invoke(refresh, ['L:8080:localhost:80', '--delay', '3'], 
                                  obj={'session': 'portmux', 'config': None, 'verbose': False})

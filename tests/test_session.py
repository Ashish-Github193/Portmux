...
from portmux.session import create_session, session_exists, kill_session
...
class TestCreateSession:
    ...
    def test_create_session_already_exists(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="duplicate session: test-session"
        )
    ...
    def test_create_session_other_error(self, mocker):
        ...
        with pytest.raises(TmuxError, match="Failed to create session 'test-session': some other error"):
...
class TestKillSession:
    ...
    def test_kill_session_not_found(self, mocker):
        ...
        mock_run.return_value = MagicMock(
            returncode=1, 
            stderr="session not found: test-session"
        )
    ...
    def test_kill_session_other_error(self, mocker):
        ...
        with pytest.raises(TmuxError, match="Failed to kill session 'test-session': some other error"):

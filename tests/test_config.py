...
from portmux.config import (
    load_config, validate_config, get_default_identity, DEFAULT_CONFIG
)
...
class TestValidateConfig:
    ...
    def test_validate_config_missing_session_name(self):
        ...
        with pytest.raises(ConfigError, match="Missing required config key: 'session_name'"):
            validate_config(config)

    def test_validate_config_invalid_session_name_type(self):
        config = {'session_name': 123}
        
        with pytest.raises(ConfigError, match="'session_name' must be a non-empty string"):
            validate_config(config)

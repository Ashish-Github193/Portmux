...
from .exceptions import PortMuxError, TmuxError, SSHError, ConfigError
...
def handle_error(error: PortMuxError) -> None:
    ...
    if isinstance(error, TmuxError):
        ...
        if "not installed" in str(error):
            console.print("[yellow]Hint:[/yellow] Install tmux with your package manager")
...
def create_forwards_table(forwards: List[Dict[str, Any]], include_status: bool = True) -> Table:
...
def validate_direction(direction: str) -> str:
    ...
    if direction not in ('L', 'R', 'LOCAL', 'REMOTE'):
        raise click.BadParameter(f"Invalid direction '{direction}'. Must be 'L'/'LOCAL' or 'R'/'REMOTE'")

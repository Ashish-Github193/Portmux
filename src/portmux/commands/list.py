...
from ..utils import handle_error, create_forwards_table
...
@click.option('--status', 'include_status', is_flag=True, help='Include connection status')
@click.pass_context
def list(ctx: click.Context, output_json: bool, include_status: bool):
    ...
    try:
        ...
        if output_json:
            ...
            if not forwards:
                console.print(f"[yellow]No active forwards in session '{session_name}'[/yellow]")
                console.print("[blue]Use 'portmux add' to create new forwards[/blue]")
            else:
                if verbose:
                    console.print(f"[blue]Active forwards in session '{session_name}':[/blue]\n")

...
from ..forwards import refresh_forward, list_forwards
...
def refresh(ctx: click.Context, name: str, refresh_all: bool, delay: float):
    ...
    try:
        ...
        if refresh_all:
            if not forwards:
                console.print(f"[yellow]No forwards to refresh in session '{session_name}'[/yellow]")
                return
                
            console.print(f"[blue]Refreshing all {len(forwards)} forward(s) with {delay}s delay...[/blue]")
            ...
            with Progress(
                ...
                for i, forward in enumerate(forwards):
                    ...
                    try:
                        ...
                        refreshed_count += 1
                        if verbose:
                            console.print(f"[green]Refreshed forward '{forward['name']}'[/green]")
                        ...
                        console.print(f"[red]Failed to refresh '{forward['name']}': {e}[/red]")
                    
                    progress.remove_task(task)
            
            console.print(f"[green]Successfully refreshed {refreshed_count}/{len(forwards)} forward(s)[/green]")
            return
        ...
        if verbose or delay != config.get('reconnect_delay', 1):
            console.print(f"[blue]Refreshing forward '{name}' with {delay}s delay...[/blue]")

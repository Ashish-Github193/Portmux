...
from ..session import create_session, session_exists, kill_session
...
@click.command()
@click.option('--force', '-f', is_flag=True, help='Force recreation of existing session')
@click.pass_context
def init(ctx: click.Context, force: bool):
    ...
    try:
        ...
        try:
            ...
            if verbose:
                console.print(f"[green]Configuration loaded from {get_config_path()}[/green]")
        except Exception:
            if verbose:
                console.print("[yellow]Creating default configuration...[/yellow]")
            create_default_config()
            config = load_config(config_path)
            console.print(f"[green]Default configuration created at {get_config_path()}[/green]")
        
        # Check if session exists
        if session_exists(session_name):
            if force:
                console.print(f"[yellow]Destroying existing session '{session_name}'...[/yellow]")
        ...
        if success:
            console.print(f"[green]Successfully initialized PortMUX session '{session_name}'[/green]")
